          ...
          {categories.map((cat, i) => (
            <ScrollReveal key={i}>
              <div className="group p-8 md:p-12 rounded-[40px] border border-[var(--fun-stroke-1)] bg-[var(--fun-card)] hover:border-[var(--fun-purple)] transition-all flex flex-col md:flex-row items-start md:items-center gap-8">
                <div
                  className={`h-24 w-24 md:h-36 md:w-36 rounded-3xl flex items-center justify-center border border-[var(--fun-stroke-1)] group-hover:scale-110 transition-transform shrink-0 overflow-hidden ${cat.slug === "quakesafe" ? "bg-[#0F172A]" : "bg-white p-4"}`}
                >
                  <img
                    src={cat.logo}
                    alt={cat.name}
                    className={`max-w-full max-h-full object-contain ${cat.slug === "quakesafe" ? "w-full h-full scale-100" : "scale-125"}`}
                  />
                </div>
