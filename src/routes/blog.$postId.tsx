...
const LANG_LABELS: Record<string, string> = {
  tr: "Türkçe", en: "English", az: "Azerice", de: "Deutsch", fr: "Français", es: "Español",
  ru: "Русский", ar: "العربية", it: "Italiano", pt: "Português", ja: "日本語", zh: "中文"
};
...
function PostPage() {
  ...
  const fetchPost = useCallback(async (isInitial = true) => {
    ...
    try {
      ...
      if (data) {
        ...
        let text = data[`text_${lang}`] || data[`content_${lang}`] || data.text || data.content || data.description;
  ...
  if (!post) return (
    <div className="flex flex-col items-center justify-center min-h-screen text-center px-4">
      ...
      <ArrowButton to="/blog" variant="dark">{t("error.home")}</ArrowButton>
    </div>
  );
    ...
    <main className="pt-32 pb-20 px-4 lg:px-5 bg-[var(--color-background)]">
      <div className="max-w-[900px] mx-auto">
        ...
        <div className="mb-12 flex items-center justify-between relative">
          <ArrowButton to="/blog" variant="light" direction="left" className="!py-2 !px-4 !text-sm border border-[var(--fun-stroke-1)]">
          ...
          <div className="flex items-center gap-3" ref={menuRef}>
            ...
            {menuOpen && (
              <div className="absolute right-0 top-full mt-2 w-56 rounded-2xl border border-[var(--fun-stroke-1)] bg-[var(--fun-card)] shadow-2xl z-50 p-2 animate-in slide-in-from-top-2 duration-200">
                ...
                <button
                  onClick={() => { setLangMenuOpen(true); setMobileMenuOpen(false); }}
                ...
                >
                  ...
                  <span className="uppercase text-[10px] font-bold bg-[var(--fun-surface)] px-2 py-0.5 rounded-md">{lang}</span>
                </button>
              </div>
            )}
            ...
            {langMenuOpen && (
              <div className="absolute right-0 top-full mt-2 w-64 max-h-[400px] overflow-y-auto rounded-2xl border border-[var(--fun-stroke-1)] bg-[var(--fun-card)] shadow-2xl z-50 p-2 animate-in zoom-in-95 duration-200 custom-scrollbar">
                ...
                {Object.entries(LANG_LABELS).map(([code, label]) => (
                  <button
                    key={code}
                    onClick={() => { setLang(code as Lang); setLangMenuOpen(false); }}
                  ...
                  >
                    <div className="flex items-center gap-3">
                       <img src={FLAG_MAP[code]} className="h-5 w-5 rounded-full object-cover border border-white/20" alt="" />
                       {label}
                    </div>
          ...
          <div className="relative">
            {contentLoading && (
              <div className="absolute inset-0 z-10 space-y-6 bg-[var(--color-background)]/80 backdrop-blur-sm animate-in fade-in duration-300">
                 {[...Array(8)].map((_, i) => (
                   <div key={i} className={`h-6 bg-[var(--fun-surface)] rounded-full animate-pulse ${i % 3 === 0 ? 'w-full' : i % 3 === 1 ? 'w-5/6' : 'w-4/6'}`}></div>
                 ))}
              </div>
            )}
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-8 pt-12 border-t border-[var(--fun-stroke-1)]">
             <div className="flex items-center gap-4">
                ...
                <div>
                   ...
                   <p className="text-sm font-bold fun-text-muted uppercase tracking-widest">{t("blog.post.author_label")}</p>
                </div>
             </div>
        ...
        <ScrollReveal>
          <div className="mt-20 flex flex-col md:flex-row items-center justify-between gap-8 bg-[var(--fun-surface)] p-10 rounded-[40px] border border-[var(--fun-stroke-1)]">
             ...
             <ArrowButton to="/blog" variant="dark">{t("blog.post.all_posts")}</ArrowButton>
          </div>
        </ScrollReveal>
      </div>
    </main>
