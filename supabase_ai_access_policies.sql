...
-- ai_support_feedback (Sadece sahibi görebilir/okuyabilir, oturum açmış olması gerekir)
DROP POLICY IF EXISTS "Ai Erişimi" ON ai_support_feedback;

CREATE POLICY "ai_support_feedback_select_policy" ON ai_support_feedback
FOR SELECT TO authenticated
USING (user_id::text = auth.uid()::text);

CREATE POLICY "ai_support_feedback_insert_policy" ON ai_support_feedback
FOR INSERT TO authenticated
WITH CHECK (user_id::text = auth.uid()::text);


-- 4. GEÇMİŞ DESTEK TALEPLERİ TABLOSU (past_support_tickets)
...
CREATE POLICY "Ai Erişimi" ON past_support_tickets
